...
def fetch_poster(movie_name: str) -> tuple:
    """Returns (poster_url, rating, year)."""
...
def fetch_anime_poster(anime_name: str) -> tuple:
    ...
    if cache_key in _poster_cache:
        return _poster_cache[cache_key]
    try:
        # We don't always know if it's a movie or series for the omdb call, 
        # but omitting type usually works or we can just try series then movie.
        for key in OMDB_KEYS:
            url = f"http://www.omdbapi.com/?t={requests.utils.quote(anime_name)}&apikey={key}"
            data = requests.get(url, timeout=5).json()
            if data.get("Response") == "True":
                poster = data.get("Poster") or FALLBACK_POSTER
                if poster == "N/A":
...
def get_fallback_recommendations(movie: str, n: int = 10) -> list:
    try:
...
def get_recommendations(movie: str) -> dict:
    """Return 4 recommendation sections."""
    return {
        "by_tags":   recommend(movie, "similarity_tags_tags.pkl"),
        "by_genres": recommend(movie, "similarity_tags_genres.pkl"),
        "by_cast":   recommend(movie, "similarity_tags_tcast.pkl"),
        "by_crew":   recommend(movie, "similarity_tags_tcrew.pkl"),
    }
...
def get_person_details(name: str) -> dict:
    ...
    try:
        ...
        known_for = movies_df[movies_df["has_person"]]["title"].tolist()[:8]
    except Exception:
        known_for = []
